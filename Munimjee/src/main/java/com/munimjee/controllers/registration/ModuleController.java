...
public class ModuleController {
	...
	public ResponseEntity<ModelModules> addModule(@RequestBody ModelModules module,UriComponentsBuilder ucBuilder)
	{
		...
		if(moduleDao.exists(module))
		{
			...
			return new ResponseEntity<ModelModules>(HttpStatus.CONFLICT);
		}
		module = moduleDao.add(module);
		if(module.getModuleId()==0)
		{
			return new ResponseEntity<ModelModules>(HttpStatus.EXPECTATION_FAILED);
		}
		 return new ResponseEntity<ModelModules>(module, HttpStatus.CREATED);
	}
	...
	{
		log.info("updateModule Service Called");
		System.out.println("addModule service called with id "+module.getModuleId());
		ModelModules temp = moduleDao.get(module.getModuleId());
		if(temp!=null)
		{
			log.info("same user exists");
